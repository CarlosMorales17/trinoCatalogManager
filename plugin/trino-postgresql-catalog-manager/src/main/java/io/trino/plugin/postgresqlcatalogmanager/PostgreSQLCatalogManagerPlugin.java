...
public final class PostgreSQLCatalogManagerPlugin
        extends JdbcCatalogManagerSpi
{
    private static final Logger log = Logger.get(PostgreSQLCatalogManagerPlugin.class);

    public PostgreSQLCatalogManagerPlugin(Map<String, String> properties)
    {
        super(properties);
        log.info("PostgreSQLCatalogManagerPlugin initialized with properties: %s", properties);
    }
    ...
    protected JdbcCatalogConfig createConfig(Map<String, String> props)
    {
        return JdbcCatalogConfig.builder()
                .jdbcUrl(props.getOrDefault("postgresql.jdbc-url", "jdbc:postgresql://localhost:5432/xpq_catalog_database"))
                .username(props.getOrDefault("postgresql.username", "trino"))
                .password(props.getOrDefault("postgresql.password", ""))
                .refreshInterval(Long.parseLong(props.getOrDefault("postgresql.refresh-interval", "3000")))
                .build();
    }
    ...
    protected JdbcSchemaMapping createSchemaMapping()
    {
        return new JdbcSchemaMapping()
        {
            ...
            /*********************************************************************
             * Methods Below are "Experimental" to support addition of new Catalogs Through Trino
             /******************************************************************************/
            ...
            public String getUpsertCatalogSql()
            {
                return """
                        ...
                        VALUES (?, ?, ?::jsonb, ?)
                        ON CONFLICT (catalog_name, version_identifier)
                        DO UPDATE SET
