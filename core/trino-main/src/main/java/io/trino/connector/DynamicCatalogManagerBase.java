...
public abstract class DynamicCatalogManagerBase
        implements CatalogManager, ConnectorServicesProvider
{
    protected Logger log;

    protected enum State { CREATED, INITIALIZED, STOPPED }

    protected final CatalogFactory catalogFactory;
     ...
     */
    protected final ConcurrentMap<CatalogName, Catalog> activeCatalogs = new ConcurrentHashMap<>();

    /**
     * All catalogs including those that have been dropped.
     */
    protected final ConcurrentMap<CatalogHandle, CatalogConnector> allCatalogs = new ConcurrentHashMap<>();

    public DynamicCatalogManagerBase(CatalogFactory catalogFactory, Executor executor)
    {
        this.catalogFactory = requireNonNull(catalogFactory, "catalogFactory is null");
        this.executor = requireNonNull(executor, "executor is null");
    }

    protected void setCatalogStore(CatalogStore catalogStore)
    {
                    ...
                    catalogStore.getCatalogs().stream()
                            .map(storedCatalog -> (Callable<?>) () -> {
                                ...
                                try {
                                    ...
                                    CatalogHandle catalogHandle = catalog != null ? catalog.catalogHandle() : createRootCatalogHandle(storedCatalog.name(), new CatalogHandle.CatalogVersion("failed"));
    ...
    public void pruneCatalogs(Set<CatalogHandle> catalogsInUse)
    {
        ...
        try {
            ...
            Iterator<Map.Entry<CatalogHandle, CatalogConnector>> iterator = allCatalogs.entrySet().iterator();
            while (iterator.hasNext()) {
                Map.Entry<CatalogHandle, CatalogConnector> entry = iterator.next();
    ...
    public void createCatalog(CatalogName catalogName, ConnectorName connectorName, Map<String, String> properties, boolean notExists)
    {
        ...
        try {
            ...
            CatalogProperties catalogProperties = catalogStore
    ...
    public void dropCatalog(CatalogName catalogName, boolean exists)
    {
        ...
        try {
            ...
            if (this.removeCatalogsInstantly) {
                ...
                removed = activeCatalogs.containsKey(catalogName);
            }
        }
        finally {
            catalogsUpdateLock.unlock();
